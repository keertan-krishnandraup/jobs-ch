...
async def get_meta1(query_q):
    qstr = await query_q.get()
    print(qstr)
    url = 'https://www.jobs.ch/api/v1/meta/place/type-ahead?rows=5000&subTypes[0]=city&term='+str(qstr)
    ...
    consul1_coll = harvests_db['consul1b']
    for j in json_sp:
...
def kw_driver(process_queue_size, query_q):
    ...
    loop.run_until_complete(make_tasks_and_exc(query_q, process_queue_size, 3, get_meta1))
